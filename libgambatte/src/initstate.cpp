...
void gambatte::setInitState(SaveState &state, bool const cgb, bool const gbaCgbMode, bool const clearSram) {
	...
	/* Sachen MMC1 lock-progress counter; harmless default for non-Sachen carts. */
	state.mem.sachenLockCount = 0;
   ...
   //set initial gbc gb mode bg palette
   for (int i = 0; i < 8; i += 2) {
      state.ppu.dmgPalette[i    ] = 0xFF;
      state.ppu.dmgPalette[i + 1] = 0x7F;
   }
